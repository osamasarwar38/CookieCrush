...
bool flag=false;
float timeee=100;        // deals with the time
//===========================CHECK FOR MATCHES==========================
...
void checkMatchesX(int arr[9][14]){
	...
	for (int i=0;i<9;i++){
		for (int j=0;j<14;j++){
			int checkX=j+1;
			int matchesX=0;
			while ((arr[i][j]==arr[i][checkX] && checkX<14 && arr[i][j]!=10)){
					matchesX++;
					checkX++;}
			if (matchesX>=2){
				score+=matchesX+1;
				...
				while (tempArr==arr[i][tempj] && tempj<14){
					arr[i][tempj]=20;            //same pair of cookies are assigned the value of 20
					tempj++;}
					}
			}
	}
...
void checkMatchesY(int arr[9][14]){
		...
		for (int i=0;i<9;i++){
			int checkY=i+1;
			int matchesY=0;
			while ((arr[i][j]==arr[checkY][j]) && checkY<9 && arr[i][j]!=10 ){
					matchesY++;
					checkY+=1;}
					if (matchesY>=2){
						score+=matchesY+1;
						int tempi=i;
						int tempArr=arr[i][j];
						while (tempArr==arr[tempi][j] && tempi<9){
							arr[tempi][j]=20;         //same pair of cookies are assigned the value of 20
							tempi+=1;}	
						}			
				}
		}
...
// #----------------- Write your code here ----------------------------#
	...
	if (play==false){
		...
		DrawString(20, height - 20, width, height,
			"Cookie-Crush Final-Version", colors[RED]);
			DrawString(200,300, width, height,
			"Click anywhere to start the game... ", colors[RED]);
	}
    else{                                 //When the game starts	
		arrray();
		timeee-=0.5;
		if (timeee<=0)timeee=0;
		...
		for (int y=height-120,i=0 ; y>=60 ;y-=60,i+=1){
			for (int x=60,j=0 ;x<width-60; x+=60,j+=1){
				...
				if (arr[i+1][j]==10 || arr[i+1][j]==20){
					arr[i+1][j]=arr[i][j];
					arr[i][j]=10;                //The cookies above the same cookies are assigned the value of 10
					}
				if (arr[i][j]!=10 && arr[i][j]!=20)
					DrawCookie(random_cookie[arr[i][j]], x, y, cwidth, cheight);
			}
		}
		
		DrawString(width  - 150, height - 20, width, height,
			"Time Left " + Num2Str(round(timeee)), colors[RED]);	
		DrawString(width / 2 - 20, height - 20, width, height,
			"Score " + Num2Str(score), colors[BLUE_VIOLET]);

if (timeee<=0){                 //When the time is up
		gameStatus="GameOver";
		DrawString(200, 20, width, height,
...
//         ------------SWAP IN X DIRECTION---------------	
	...
	//-----------------------------------------------------------------------					   
					                                   //When we try to swap from the other cookie 
					 if	 ( easily==false){
						int i=elemY-1;
...
void Timer(int m) {
	dt = (m - lastframe) / 500.0;
	lastframe = m;
	// dt is time elapsed between two frames..
	glutPostRedisplay();
	// implement your functionality here
	glutTimerFunc(500, Timer, m + 1);
}
